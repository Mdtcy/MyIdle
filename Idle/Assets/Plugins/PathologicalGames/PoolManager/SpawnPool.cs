...
namespace PathologicalGames
{
    ...
    public sealed class SpawnPool : MonoBehaviour, IList<Transform>
    {
        ...
        // ---------- jie.wen 20201123 ----------
        /// <summary>
        /// 如果Spawn传入的position = Vector3.Zero，使用Pool的position设置其值
        /// </summary>
        public bool usePoolPosition = true;

        /// <summary>
        /// 如果Spawn传入的rotation = Quaternion.identity，使用Pool的rotation设置其值
        /// </summary>
        public bool UsePoolRotation = true;
        // ---------- jie.wen 20201123 ----------
        ...
        public GameObject SpawnAsGameObject(GameObject prefab)
        {
            return Spawn(prefab).gameObject;
        }

        public GameObject SpawnAsGameObject(GameObject prefab, Transform parent)
        {
            return Spawn(prefab, parent).gameObject;
        }
    ...
    public class PrefabPool
    {
        ...
        internal Transform SpawnInstance(Vector3 pos, Quaternion rot)
        {
            ...
            if (this._despawned.Count == 0)
            {
                ...
                // jie.wen 20201125 spawnNew may call InstantiateDelegate, and the returned instance's active state may be false.
        ...
        public Transform SpawnNew(Vector3 pos, Quaternion rot)
        {
            ...
            if (pos == Vector3.zero && this.spawnPool.usePoolPosition)
            {
                pos = this.spawnPool.group.position;
            }
            if (rot == Quaternion.identity && this.spawnPool.UsePoolRotation)
            {
