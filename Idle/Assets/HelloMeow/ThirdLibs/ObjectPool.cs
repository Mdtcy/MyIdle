...
namespace HM.GameBase {
	public interface IHMPooledObject {
		void OnEnterPool ();
	}

	/** Lightweight object Pool for IHMPooledObject.
	 * Handy class for pooling objects of type T which implements the IHMPooledObject interface.
	 *
	 * Usage:
	 ...
	 */
	public static class ObjectPool<T> where T : class, IHMPooledObject, new()
	{
	...
	{
		public static void ReleaseToPool<T>(this T obj) where T : class, IHMPooledObject, new()
		{
			ObjectPool<T>.Release(ref obj);
		}
	}
	...
	public static class ObjectPoolSimple<T> where T : class, new(){
		 ...
		 * However enabling HM_OPTIMIZE_POOLING will prevent this check.
		 *
		 * \see Claim
		 */
		public static void Release (ref T obj) {
		...
		public static void Clear () {
			lock (pool) {
#if !HM_OPTIMIZE_POOLING && !HM_NO_POOLING
				inPool.Clear();
#endif
				pool.Clear();
			}
